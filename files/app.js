...
async function userManagementAction(action, body, successMessage) {
  try {
    if (remoteMode) await apiRequest(action, { method: 'POST', body: JSON.stringify(body) });
      ...
      if (action === 'delete_role') { const index = localAuth.roles.findIndex(item => item.id === body.roleId); const role = localAuth.roles[index]; if (!role || role.roleKey === 'admin' || role.isSystem || localAuth.users.some(user => user.roleId === body.roleId && !user.deletedAt)) throw new Error('Il ruolo non può essere eliminato: è di sistema o assegnato a un utente.'); localAuth.roles.splice(index, 1); }
...
function renderInstitutionMembers(type, record) {
  ...
  container.innerHTML = institutionSettings(type).roles.map(role => {
    const members = membersList.filter(member => member.role === role.id && matchesArchiveSearch([member.name, member.annotations], query)).sort((a, b) => (a.endDate ? 0 : 1) - (b.endDate ? 0 : 1) || String(a.endDate || a.startDate || '').localeCompare(String(b.endDate || b.startDate || '')));
    const active = members.filter(institutionActive).length;
    return `<section class="mb-4"><h4 class="h6">${escapeHtml(role.name)}</h4><div class="parliament-member-list">${members.length ? members.map(member => `<div class="member-sequence-item"><div class="parliament-member ${member.endDate ? 'is-resigned' : ''}"><div><strong>${escapeHtml(member.name)}</strong><span class="d-block small text-secondary">${member.endDate ? `Cessato il ${formatDate(member.endDate)}` : `Nominato il ${formatDate(member.startDate)}`}${member.annotations ? ` · Annotazioni: ${escapeHtml(member.annotations)}` : ''}</span></div><div class="d-flex gap-2"><button type="button" class="btn btn-sm btn-outline-secondary" data-edit-institution-member="${type}" data-member-id="${member.id}">Modifica</button>${!member.endDate ? `<button type="button" class="btn btn-sm btn-outline-danger" data-end-institution-member="${type}" data-member-id="${member.id}">Cessa</button>` : ''}${can(institutionConfigs[type].view, 'delete') ? `<button type="button" class="btn btn-sm btn-outline-danger" data-trash-item="${type === 'government' ? 'governmentMembers' : 'compositionMembers'}" data-entity-id="${member.id}" data-parent-id="${record.id}">Cestino</button>` : ''}</div></div></div>`).join('') : `<p class="small text-secondary mb-2">Nessun componente.</p>`}${active < role.limit ? `<button type="button" class="btn btn-sm btn-outline-primary" data-add-institution-member="${type}" data-role-id="${role.id}">+ Aggiungi ${escapeHtml(role.name)}</button>` : ''}</div></section>`;
  }).join('');
}
...
function renderMemberList(mandate, role) {
  ...
  const members = mandate.members.filter(member => member.role === role && matchesArchiveSearch([member.name, member.memberParty, member.memberCoalition, member.annotations], query)).sort((a, b) => (a.resignationDate ? 0 : 1) - (b.resignationDate ? 0 : 1) || String(a.resignationDate || a.oathDate || '').localeCompare(String(b.resignationDate || b.oathDate || '')));
...
async function initialize() {
  ...
  document.addEventListener('click', event => { const createRoleButton = event.target.closest('#createRoleButton'); if (createRoleButton) userManagementAction('create_role', { name: document.getElementById('newRoleName').value }, 'Ruolo creato.'); const deleteRoleButton = event.target.closest('.delete-role-button'); if (deleteRoleButton) { if (confirm('Eliminare definitivamente questo ruolo? L’operazione non può essere annullata.')) userManagementAction('delete_role', { roleId: deleteRoleButton.dataset.roleId }, 'Ruolo eliminato.'); return; } const savePermissionsButton = event.target.closest('.save-role-permissions'); if (savePermissionsButton) { const permissions = {}; document.querySelectorAll(`.role-permission[data-role-id="${savePermissionsButton.dataset.roleId}"]`).forEach(input => { permissions[input.dataset.permission] ||= {}; permissions[input.dataset.permission][input.dataset.action] = input.checked; }); userManagementAction('save_role_permissions', { roleId: savePermissionsButton.dataset.roleId, permissions }, 'Permessi salvati.'); } });
