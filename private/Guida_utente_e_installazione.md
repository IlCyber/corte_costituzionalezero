...
5. Modificare le costanti `DB_HOST`, `DB_NAME`, `DB_USER` e `DB_PASSWORD` in `private/config.php`, non in `api.php`.
6. Caricare `index.html`, `styles.css`, `app.js`, `api.php` e la cartella `private/`. Il file SQL può essere rimosso dal sito dopo l'importazione.
